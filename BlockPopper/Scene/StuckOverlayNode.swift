...
class StuckOverlayNode: SKNode {
    ...
    func show(livesRemaining: Int) {
        self.watchAdButton.isHidden = livesRemaining <= 0
        self.watchAdLabel.isHidden = livesRemaining <= 0

        if livesRemaining <= 0 {
            self.messageLabel.text = "Game Over!"
            self.descriptionLabel.text = ""
            self.livesInfoLabel.text = "No lives remaining"
        } else {
            self.messageLabel.text = "You're Stuck!"
            self.descriptionLabel.text = "Clear two lines and two rows to continue?"
            self.livesInfoLabel.text = "\(livesRemaining) lives remaining"
        }
